...
if [ -z "$REPO" ]; then
  echo "❌ Impossible de détecter le repo. Lance 'gh auth login' d'abord."
  exit 1
fi
...
echo "✅ Secrets SSH ajoutés"

# ── 2. Service Principal Azure ────────────────────────────────
SUBSCRIPTION_ID=$(az account show --query id -o tsv)
echo "📋 Subscription Azure : $SUBSCRIPTION_ID"

echo "🔧 Création du Service Principal Azure (taskflow-sp)..."
...
if [ -n "$SONAR_TOKEN" ]; then
  ...
  echo "✅ SONAR_TOKEN ajouté"
else
  echo "⏭️  SONAR_TOKEN ignoré (tu pourras l'ajouter plus tard)"
fi
